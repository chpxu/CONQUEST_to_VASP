"""
This file reads the output file generated by a static run of CONQUEST, which outputs atomic forces and stresses, and the Harris-Foulkes energy at the end of the file.

This is different from post-processing with ASE, which is probably easier but requires that you had `IO.WriteOutToASEFile T` before running. This file will be useful for runs done without any ASE at all.

This class may also work on output files generated from structural/atomic relaxations, but only if `IO.Iprint 2` because by default, detailed information is not printed out at the end of these runs (and frankly, only the final structure is important anyways).
