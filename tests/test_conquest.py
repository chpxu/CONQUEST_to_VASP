...
test_coords_proc = conquest_coordinates_processor("tests/data/test.dat", test_species)
...
def test_processor_file_not_found() -> None:
    with pytest.raises(FileNotFoundError):
        conquest_coordinates_processor("tests/data/does_not_exist.dat", test_species)


def test_natoms_and_atom_count_match() -> None:
    assert int(test_coords_proc.coords.natoms) == 20
    assert len(test_coords_proc.coords.atoms) == 20


def test_lattice_vectors_parsed() -> None:
    lattice = test_coords_proc.coords.lattice_vectors
...
def test_atom_labels_assigned() -> None:
    labels = {atom.label for atom in test_coords_proc.coords.atoms}
    assert labels == {"Bi", "Mn", "O"}


def test_number_of_elements() -> None:
    counts = test_coords_proc.coords.number_of_elements()
    assert counts == {"Bi": 4, "Mn": 4, "O": 12}


def test_element_map_contents_are_atoms_of_correct_label() -> None:
    element_map = test_coords_proc.coords.element_map
...
def test_cartesian_positions_shape_and_values() -> None:
    cart = test_coords_proc.coords.get_cartesian_positions()
    assert cart.shape == (20, 3)
    first_atom = test_coords_proc.coords.atoms[0]
    # Cartesian coords = frac coords @ lattice_vectors.T
    expected = first_atom.coords @ test_coords_proc.coords.lattice_vectors.T
    assert np.allclose(first_atom.cart_coords, expected)


def test_atom_numbers_are_sequential() -> None:
    numbers = [atom.number for atom in test_coords_proc.coords.atoms]
...
def test_atom_str_contains_key_fields() -> None:
    atom = test_coords_proc.coords.atoms[0]
    rendered = str(atom)
...
def test_atom_default_forces_and_spins() -> None:
    atom = test_coords_proc.coords.atoms[5]
...
def test_atom_charge_assigns_spin_difference() -> None:
    charge_proc = atom_charge(
        test_coords_proc.coords, "tests/data/test_original_AtomCharge.dat"
    )
    # spin = up - down, stored in the z-component
    for atom, charge_row in zip(
        test_coords_proc.coords.atoms, charge_proc.conquest_charge_data
    ):
...
def test_atom_charge_file_not_found() -> None:
    with pytest.raises(FileNotFoundError):
        atom_charge(test_coords_proc.coords, "tests/data/no_such_charge_file.dat")