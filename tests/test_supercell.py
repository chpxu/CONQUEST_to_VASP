...
def test_integer_params() -> None:
    single_cell: supercell = supercell(repeats_x=1, repeats_y=1, repeats_z=1, coords=test_coords_proc)
...
def test_single_repeat_x() -> None:
    ...
    assert (new_cell.repeats_x + 1) * 5.92770000 < new_cell.supercell_coords.lattice_vectors[0][0] < (new_cell.repeats_x + 1) * 5.9290000 # account for floating point error
    assert (new_cell.repeats_y + 1) * 7.439000 < new_cell.supercell_coords.lattice_vectors[1][1] < (new_cell.repeats_y + 1) * 7.44100000 # account for floating point error
    assert (new_cell.repeats_z + 1) * 5.371999999 < new_cell.supercell_coords.lattice_vectors[2][2] < (new_cell.repeats_z + 1) * 5.372100000 # account for floating point error
...
def test_single_repeat_all() -> None:
    ...
    assert len(new_cell.supercell_coords.Atoms) == num_repeats * len(new_cell.coords.Atoms)
    assert int(new_cell.supercell_coords.natoms) == len(new_cell.supercell_coords.Atoms)
    assert (new_cell.repeats_x + 1) * 5.927799999 < new_cell.supercell_coords.lattice_vectors[0][0] < (new_cell.repeats_x + 1) * 5.9290000 # account for floating point error
    assert (new_cell.repeats_y + 1) * 7.439000 < new_cell.supercell_coords.lattice_vectors[1][1] < (new_cell.repeats_y + 1) * 7.44100000 # account for floating point error
    assert (new_cell.repeats_z + 1) * 5.371999999 < new_cell.supercell_coords.lattice_vectors[2][2] < (new_cell.repeats_z + 1) * 5.372100000 # account for floating point error
