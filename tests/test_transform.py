...
from conquest2a.conquest import Atom, conquest_coordinates, conquest_species
...
@pytest.fixture
def init_cq_proc(species: conquest_species) -> conquest_coordinates:
    return read_coords(input_cell, species, "cq").coords


@pytest.fixture
def final_cq_proc(species: conquest_species) -> conquest_coordinates:
    return read_coords(final_cell, species, "cq").coords
...
def test_same_atom_count(init_cq_proc: conquest_coordinates) -> None:
    orig_natoms = init_cq_proc.natoms
    num_atoms = len(init_cq_proc.atoms)
    tuc = transform_unit_cell(init_cq_proc)
    tuc.transform(P)
    assert int(orig_natoms) == num_atoms
    assert int(init_cq_proc.natoms) == int(orig_natoms)
    assert num_atoms == len(init_cq_proc.atoms)


def test_same_lattice_vect(init_cq_proc: conquest_coordinates) -> None:
    orig_vects = init_cq_proc.lattice_vectors
    tuc = transform_unit_cell(init_cq_proc)
    tuc.transform(P)
    # Float comparison
    assert np.allclose(init_cq_proc.lattice_vectors, orig_vects, atol=1e-7)


def test_same_atoms(init_cq_proc: conquest_coordinates) -> None:
    """
    ...
    """
    orig_atoms = copy.deepcopy(init_cq_proc.atoms)
    tuc = transform_unit_cell(init_cq_proc)
    tuc.transform(P)
    # Float comparison
    for i in range(1, len(orig_atoms)):
        assert orig_atoms[i].label == init_cq_proc.atoms[i].label
        assert orig_atoms[i].number == init_cq_proc.atoms[i].number
        assert orig_atoms[i].species == init_cq_proc.atoms[i].species
        assert orig_atoms[i].can_move == init_cq_proc.atoms[i].can_move
        assert np.allclose(init_cq_proc.atoms[i].coords, orig_atoms[i].coords, atol=1e-7)


# null test: identity leaves everything unchanged


def test_identity_same_atom_count(init_cq_proc: conquest_coordinates) -> None:
    num_atoms = len(init_cq_proc.atoms)
    tuc = transform_unit_cell(init_cq_proc)
    tuc.transform(identity)
    assert int(tuc.transformed_cell_coords.natoms) == num_atoms
    assert int(init_cq_proc.natoms) == int(tuc.transformed_cell_coords.natoms)
    assert num_atoms == len(tuc.transformed_cell_coords.atoms)


def test_identity_same_lattice_vect(init_cq_proc: conquest_coordinates) -> None:
    orig_vects = init_cq_proc.lattice_vectors
    tuc = transform_unit_cell(init_cq_proc)
    tuc.transform(identity)
    # Float comparison
...
def test_identity_same_atoms(init_cq_proc: conquest_coordinates) -> None:
    """
...
def test_supercell_expansion_multiplies_atom_count(init_cq_proc, scale_matrix, multiplier):
    original_count = len(init_cq_proc.atoms)
    transformer = transform_unit_cell(init_cq_proc)
...
def test_P_atom_count(
    init_cq_proc: conquest_coordinates, final_cq_proc: conquest_coordinates
) -> None:
    num_atoms = len(init_cq_proc.atoms)
    tuc = transform_unit_cell(init_cq_proc)
    tuc.transform(P)
    ...
    assert int(init_cq_proc.natoms) == 2 * int(tuc.transformed_cell_coords.natoms)
    assert len(init_cq_proc.atoms) == num_atoms
    assert int(init_cq_proc.natoms) == 2 * int(tuc.transformed_cell_coords.natoms)


def test_P_lattice_vect(
    init_cq_proc: conquest_coordinates, final_cq_proc: conquest_coordinates
) -> None:
    final_vects = np.array(
        [
            [14.3929907019, 0.0000000000, 0.0000000000],
    ...
    tuc = transform_unit_cell(init_cq_proc)
    tuc.transform(P)
    assert np.allclose(final_cq_proc.lattice_vectors, final_vects, atol=1e-7)
    assert np.allclose(
        final_cq_proc.lattice_vectors, tuc.transformed_cell_coords.lattice_vectors, atol=1e-7
    )


def test_P_atoms(init_cq_proc: conquest_coordinates, final_cq_proc: conquest_coordinates) -> None:
    """
    ...
    tuc.transform(P)
    new_unit_cell = tuc.transformed_cell_coords
    expected_atoms = final_cq_proc.atoms
